"""
asteroid-collision
ref: https://leetcode.com/problems/asteroid-collision/description/?envType=study-plan-v2&envId=leetcode-75
...
class Solution(object):
    def asteroidCollision(self, asteroids):
        ...
        '''
        solution:
        1) find [+, -] pair, remove smaller item and only keep bigger item; repeat
        2) find [+, +], if left one is bigger, remove right one; repeat
        3) find [-, -], if right one is bigger, remove left one; repeat
        '''
        if not asteroids:
            return None
        arr = asteroids

        found = False # found such case to process
        ## 1) find [+, -] pair
        found = True
        while found:
            found = False
            ...
            for i in range(N-1):
                if arr[i]>0 and arr[i+1]<0:
                    ...
                    if abs(arr[i])>abs(arr[i+1]):
                        del arr[i+1] # arr.pop(i+1)
                    elif abs(arr[i])<abs(arr[i+1]):
                        del arr[i] # arr.pop(i)
                    else:
                        del arr[i:i+2] # arr.pop(i)
                    break
...
if __name__ == "__main__":
    ...
    asteroids = [3,5,-6,2,-1,4] #Output: [-6,2,4]
    print(Solution().asteroidCollision(asteroids))